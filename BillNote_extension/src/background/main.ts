...
async function handleBilinoteStart(data: { url: string; title?: string }, senderTabId?: number) {
  const result = await startTask(data.url, data.title)
  if (result.ok)
    await openSidePanelInTab(senderTabId)
  return result
}

onMessage<{ url: string; title?: string }, 'bilinote-start'>('bilinote-start', async ({ data, sender }) => {
  return handleBilinoteStart(data, sender?.tabId)
})

browser.runtime.onMessage.addListener((message, sender) => {
  if (message?.type !== 'bilinote-start')
    return undefined
  return handleBilinoteStart(message.data, sender.tab?.id)
})

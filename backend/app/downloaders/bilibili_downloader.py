...
class BilibiliDownloader(Downloader, ABC):
    ...
    def download(
        ...
        if cached_audio_path and os.path.exists(cached_audio_path):
            logger.info("音频缓存存在，重新提取元信息以获取视频标题: %s", video_id)
            try:
                info_opts = {
                    'skip_download': True,
                    'quiet': True,
                ...
                if self._cookiefile:
                    info_opts['cookiefile'] = self._cookiefile
                apply_ffmpeg_location(info_opts)
                with yt_dlp.YoutubeDL(info_opts) as ydl:
                    info = ydl.extract_info(video_url, download=False)
                return AudioDownloadResult(
                    file_path=cached_audio_path,
                ...
                logger.warning("读取 B 站标题失败，回退视频 ID: %s", exc)
                return AudioDownloadResult(
                    file_path=cached_audio_path,
                    title=video_id,
                    duration=0,
                    ...
                    platform="bilibili",
                    video_id=video_id,
                    raw_info={"id": video_id, "title": video_id},
                    video_path=None,
                )

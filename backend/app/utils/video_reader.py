...
class VideoReader:
    ...
    def _extract_single_frame(self, ts: int):
        """提取单帧，返回输出路径或 None（失败时）。"""
        time_label = self.format_time(ts)
        output_path = os.path.join(self.frame_dir, f"frame_{time_label}.jpg")
        cmd = [ffmpeg_executable(), "-ss", str(ts), "-i", self.video_path, "-frames:v", "1", "-q:v", "2", "-y", output_path,
    ...
    def extract_frames(self, max_frames=1000) -> list[str]:
        ...
        try:
            ...
            duration = float(ffmpeg.probe(self.video_path, cmd=ffprobe_executable())["format"]["duration"])

...
def _has_model_bin_in_snapshots(base_dir: Path, repo_id: str) -> bool:
    from app.transcriber.whisper_models import hf_cache_dirname

    snapshots_dir = base_dir / hf_cache_dirname(repo_id) / "snapshots"
...
def _check_whisper_model_exists(model_size: str, subdir: str = "whisper") -> bool:
    ...
    先把模型名 resolve 成可加载标识，再按类型判定：
      ...
      - HF repo_id → 看项目模型目录和全局 HF cache 布局
        <cache_root>/models--{org}--{name}/snapshots/<hash>/model.bin
    ...
    if _has_model_bin_in_snapshots(model_dir, target):
        return True

    hf_cache = os.getenv("HF_HUB_CACHE")
    if hf_cache and _has_model_bin_in_snapshots(Path(hf_cache), target):
        return True

    default_hf_cache = Path.home() / ".cache" / "huggingface" / "hub"
    if default_hf_cache != model_dir and _has_model_bin_in_snapshots(default_hf_cache, target):
        return True
...
def get_transcriber_models_status():
    ...
    # 也检查 mlx-whisper（仅 macOS）。MLX 是可选依赖，未安装时不能影响
    # 整个状态接口；前端应收到 mlx_available=false，而不是 500000。
    ...
    if mlx_available:
        try:
            ...
            logger.warning(f"MLX Whisper 未安装，跳过 MLX 模型状态检查: {e}")
            mlx_available = False
        else:

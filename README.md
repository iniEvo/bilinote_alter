...
如果你历史上混用过根目录启动和 `backend/` 目录启动，可能同时存在 `bili_note.db` 和 `backend/bili_note.db` 两份库。当前代码会默认统一使用根目录 `bili_note.db`，并兼容仅存在旧库的情况。

若需要把旧库补并到新库：
- 先修复旧任务明显错误（支持预览）：`python3 backend/scripts/fix_legacy_task_rows.py --dry-run`
- 实际修复旧任务：`python3 backend/scripts/fix_legacy_task_rows.py`
- 全量合并（provider/model/task）：`python3 backend/scripts/merge_legacy_db.py`
- 只合并自定义 provider/model：`python3 backend/scripts/merge_legacy_db.py --custom-only --skip-tasks`
- 合并后校验遗留差异：`python3 backend/scripts/check_legacy_db_merge.py`
...
说明：源码部署时默认将 Markdown 笔记写到 `backend/note_results`，并将任务结果、转写、音频缓存、状态文件等 JSON 写到 `backend/json_results`（由根目录 `.env` 的 `NOTE_OUTPUT_DIR` / `JSON_OUTPUT_DIR` 控制），这样无论你是从仓库根目录启动还是先 `cd backend` 启动，输出目录都保持一致。
